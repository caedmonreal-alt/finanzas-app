  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        {project.kind === "obra" ? (
          <>
            <K label={client ? `Le queda a ${client.name}` : "Recibido directo"} value={client && clientAvailable !== null ? formatMXN(Math.abs(clientAvailable)) : formatMXN(received)} foot={client && cb ? `${clientAvailable! >= 0 ? "le queda" : "negativo · lo puse de mi bolsa"} · me dio ${formatMXN(cb.received)} · aplicado ${formatMXN(cb.applied + cb.petty_pending + cb.loans_out)}` : contract ? `${pctContract.toFixed(0)} % de ${formatMXN(contract)}` : "Sin cliente asignado"} cls={client && clientAvailable !== null ? (clientAvailable >= 0 ? "text-positive" : "text-danger") : undefined} />
            <K label="Gastado en esta obra" value={formatMXN(spent + (petty - proved))} foot={`gastado ${formatMXN(spent)} + caja chica sin comprobar ${formatMXN(petty - proved)}${cb && cb.received ? ` · ${(((spent + petty - proved) / cb.received) * 100).toFixed(0)} % del fondo` : ""}`} />
