...
export default async function ImportarPage() {
  const [projects, people, accounts] = await Promise.all([getProjects(), getPeople(), getAccountBalances()]);
  return (
    <>
      ...
      <Importer projects={projects} people={people} accounts={accounts.filter((a) => a.type === "cash").map((a) => ({ id: a.account_id, name: a.name }))} />
    </>
  );
}
